...
class DerivativesNews extends Component {
  constructor(){
    super();
    this.state = {
      data: [],
  ...
  fetchData(){
    axios.get(`http://www.newsbtc.com/wp-json/wp/v2/posts/?categories=12040&per_page=30&page=1&_embed`)
      .then((res) => {
  ...
  render(){
    if(this.props.detailNews.isDetail){
      return <BitcoinDetailPage data={this.state.data} />;
    } else {
      return(
...
export default connect(mapStateToProps, { goDetailNews, backNews, likedPost, disLikesPost, likedDetailNews, fetchData2 })(DerivativesNews);
